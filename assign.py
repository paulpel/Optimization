...
def assign_problem(circles, flows, n):
    ...
    for idx, (i, j) in enumerate(
        ...
        problem.linear_constraints.add(
            lin_expr=[
                cplex.SparsePair(
                    ind=[x[i], x[j], y[i], y[j], d[idx]], val=[2, -2, 2, -2, -1]
                )
            ],
            senses=["L"],
            rhs=[0],
...
def generate_connected_data(
    n, min_coord=0, max_coord=10, min_radius=1, max_radius=5, max_flow=10
):
    """
    ...
    for _ in range(n):
        while True:
            ...
            if len(circles) == 0 or any(
                np.sqrt((circle[0] - c[0]) ** 2 + (circle[1] - c[1]) ** 2)
                <= circle[2] + c[2]
                for c in circles
            ):
